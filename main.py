...
class ParserClient:
    ...
    def _resolve_ref(self, ref):
        """Принимает @username (str) или числовой ID (int или строка '-100...')"""
        if isinstance(ref, int):
            if ref < 0:
                from telethon.tl.types import PeerChannel
                return PeerChannel(abs(ref))
            return ref
        s = str(ref).strip()
        if s.lstrip("-").isdigit():
            val = int(s)
            if val < 0:
                from telethon.tl.types import PeerChannel
                return PeerChannel(abs(val))
            return val
        return s  # @username или просто username

    async def get_channel_info(self, ref) -> Optional[dict]:
        try:
            entity = await self.client.get_entity(self._resolve_ref(ref))
            uname = getattr(entity, "username", None)
            info = {
                "id": entity.id,
                "username": f"@{uname}" if uname else str(ref),
                "title": getattr(entity, "title", str(ref)),
                "members_count": "?",
            }
            try:
            ...
            log.error(f"get_channel_info {ref}: {e}")
            return None

    async def get_recent_posts(self, username: str, limit: int = 20) -> list[dict]:
        try:
            entity = await self.client.get_entity(self._resolve_ref(username))
            posts = []
...
async def cb_ch_add(call: CallbackQuery, state: FSMContext):
    await call.message.answer(
        "Отправьте ссылку, юзернейм или числовой ID канала:\n\n"
...
def _parse_channel_input(raw: str):
    """Возвращает числовой ID (int) или строку @username для get_entity"""
    raw = raw.strip()
    # числовой ID: -100123... или просто 123...
    clean = raw.lstrip("-")
    if clean.isdigit():
        return int(raw)
    # ссылка → @username
    username = re.sub(r"https?://t\.me/joinchat/.*", "", raw)  # инвайт-ссылки не поддерживаем
    username = re.sub(r"https?://t\.me/", "@", username).strip("/")
...
async def process_add_channel(msg: Message, state: FSMContext):
    ...
    channel_ref = _parse_channel_input(msg.text)
    ...
    try:
        ...
        info = await parser.get_channel_info(channel_ref)
        if not info:
